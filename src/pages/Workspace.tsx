...
export function Leads() {
  ...
  return (
    <Workspace title="Leads" description="Visão comercial completa dos contatos e suas etapas no funil." icon={<User className="h-5 w-5" />}>
      ...
      <Card><Table><TableHeader><TableRow><TableHead>Nome</TableHead><TableHead>WhatsApp</TableHead><TableHead>Etapa</TableHead><TableHead>Temperatura</TableHead><TableHead>Responsável</TableHead></TableRow></TableHeader><TableBody>{rows.map((contact) => <TableRow key={contact.id} className="cursor-pointer" onClick={() => navigate(`/app/chat/${contact.id}`)}><TableCell className="font-medium">{contact.push_name || 'Contato sem nome'}</TableCell><TableCell>{contact.phone_number || contact.remote_jid.split('@')[0]}</TableCell><TableCell><Badge>{contact.pipeline_stage || 'Em Espera'}</Badge></TableCell><TableCell>{classificationLabel[contact.classification || ''] || '—'}</TableCell><TableCell className="text-muted-foreground">A definir</TableCell></TableRow>)}</TableBody></Table></Card>
    </Workspace>
  )
}
