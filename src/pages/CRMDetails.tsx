...
export function LeadDetail() {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex items-start justify-between"><div><h1 className="text-2xl font-bold">{contact.push_name || 'Contato sem nome'}</h1><p className="text-sm text-muted-foreground">{formatPhoneBR(contact.phone_number) || formatPhoneBR(contact.remote_jid.split('@')[0])}</p></div><Button onClick={() => navigate(`/app/chat/${id}`)}><MessageSquare className="mr-2 h-4 w-4" />Abrir conversa</Button></div>
